...
PostDown = firewall-cmd --remove-port ${SERVER_PORT}/udp && firewall-cmd --remove-rich-rule='rule family=ipv4 source address=${FIREWALLD_IPV4_ADDRESS}/24 masquerade' && firewall-cmd --remove-rich-rule='rule family=ipv6 source address=${FIREWALLD_IPV6_ADDRESS}/24 masquerade'" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
		...
		echo "PostUp = iptables -A FORWARD -i ${SERVER_PUB_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; iptables -A FORWARD -i ${SERVER_WG_NIC} -j ACCEPT; iptables -t nat -A POSTROUTING -o ${SERVER_PUB_NIC} -j MASQUERADE; iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 85 -j DNAT --to-destination 10.66.72.2:85; iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 84 -j DNAT --to-destination 10.66.72.2:84; iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 83 -j DNAT --to-destination 10.66.72.2:83; iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 82 -j DNAT --to-destination 10.66.72.2:82; iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 81 -j DNAT --to-destination 10.66.72.2:81; ip6tables -A FORWARD -i ${SERVER_WG_NIC} -j ACCEPT; ip6tables -t nat -A POSTROUTING -o ${SERVER_PUB_NIC} -j MASQUERADE
